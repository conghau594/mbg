...
namespace bgg
{
  ...
  void LoginScreen::doEnter() noexcept
  {
  ...
  void LoginScreen::doExit() noexcept
  {
    getServerMsgHandler().resetHandler<LoginResponse>();
  }

  void LoginScreen::update(sf::Time const &elapsed) noexcept
  {
  ...
  void LoginScreen::onWindowEventExceptClosed(
      std::optional<sf::Event> const & /*event*/) noexcept
  {
  }

  void LoginScreen::layOutScreen() noexcept
  {
  ...
  void LoginScreen::sendLoginRequest() noexcept
  {
  ...
  void LoginScreen::onLoginResponse(LoginResponse const &response) noexcept
  {
    ...
    if (errcode.value == 0) // no error -> login successfully
    {
      ...
      std::shared_ptr<IScreenInternal> gameSelectionScreen = std::make_shared<GameSelectionScreen>(
